# 工单投入量并发安全改进方案

## 改进方案1: 锁续期失败检测与任务终止

### 当前问题
```javascript
// 当前代码 - queueService.js 第639-641行
const extendLockInterval = setInterval(async () => {
  await workOrderLockManager.extendLock(workOrderId, workerId);
}, 8000); // 扩展锁但不检查结果 ❌
```

**问题**: 如果网络延迟导致锁续期失败，任务仍会继续执行，可能导致重复扣减。

### 改进代码

```javascript
// 改进后的代码
static initializeWorkOrderProcessor() {
  console.log('🔧 初始化工单数量更新处理器...');
  
  workOrderQueue.process('update-quantity', 5, async (job) => {
    const startTime = Date.now();
    const workerId = `worker_${process.pid}_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
    const { workOrderId, type, quantity, logContext, requestId } = job.data;

    let lockAcquired = false;
    let lockValid = true; // 新增：锁有效性标记
    let extendLockInterval = null; // 新增：定时器引用

    try {
      console.log(`📝 开始处理工单更新任务: ${job.id}, WorkOrder: ${workOrderId}, Type: ${type}, Quantity: ${quantity}, Worker: ${workerId}`);

      await job.progress(5);

      // 获取锁的逻辑保持不变
      console.log(`🔐 尝试获取工单锁: ${workOrderId}`);
      const lockStartTime = Date.now();
      const maxLockWaitTime = 3000;
      
      while (Date.now() - lockStartTime < maxLockWaitTime) {
        lockAcquired = await workOrderLockManager.acquireLock(workOrderId, workerId);
        if (lockAcquired) {
          break;
        }
        
        const lockStatus = await workOrderLockManager.getLockStatus(workOrderId);
        console.log(`⏳ 等待工单锁释放: ${workOrderId}, 当前持有者: ${lockStatus.owner}, 剩余时间: ${lockStatus.remainingTime}ms`);
        
        await new Promise(resolve => setTimeout(resolve, 100));
      }

      if (!lockAcquired) {
        throw new Error(`无法获取工单锁: ${workOrderId}，可能存在并发冲突或锁超时`);
      }

      await job.progress(10);

      // 导入服务
      const MaterialProcessFlowService = require('./materialProcessFlowService');
      
      await job.progress(30);
      
      // ============= 关键改进：监控锁的有效性 =============
      let lockExtensionFailureCount = 0; // 锁扩展失败计数
      const maxLockExtensionFailures = 2; // 允许的最大失败次数
      
      extendLockInterval = setInterval(async () => {
        try {
          // 检查锁扩展结果
          const extended = await workOrderLockManager.extendLock(workOrderId, workerId);
          
          if (!extended) {
            lockExtensionFailureCount++;
            console.error(`🚨 锁扩展失败 (${lockExtensionFailureCount}/${maxLockExtensionFailures}): ${workOrderId} by ${workerId}`);
            
            // 如果连续失败超过阈值，标记锁为无效
            if (lockExtensionFailureCount >= maxLockExtensionFailures) {
              lockValid = false;
              clearInterval(extendLockInterval);
              console.error(`🚨 锁已失效，将终止任务: ${workOrderId}`);
              
              // 记录告警
              await this.logLockFailure(workOrderId, workerId, job.id);
            }
          } else {
            // 扩展成功，重置失败计数
            lockExtensionFailureCount = 0;
            console.log(`✅ 锁扩展成功: ${workOrderId} by ${workerId}`);
          }
        } catch (error) {
          lockExtensionFailureCount++;
          console.error(`❌ 锁扩展异常 (${lockExtensionFailureCount}/${maxLockExtensionFailures}):`, error);
          
          if (lockExtensionFailureCount >= maxLockExtensionFailures) {
            lockValid = false;
            clearInterval(extendLockInterval);
          }
        }
      }, 5000); // 改为5秒扩展一次，更及时地检测问题
      
      try {
        // ============= 执行前检查锁是否有效 =============
        if (!lockValid) {
          throw new Error(`锁已失效，终止更新操作: ${workOrderId}`);
        }
        
        // 执行实际的数据库更新操作
        const result = await MaterialProcessFlowService._executeWorkOrderQuantityUpdate(
          workOrderId, 
          type, 
          quantity, 
          logContext
        );

        clearInterval(extendLockInterval);
        extendLockInterval = null;

        const duration = Date.now() - startTime;
        
        // ============= 任务完成后再次检查锁有效性 =============
        if (!lockValid) {
          console.error(`⚠️ 任务完成但锁在执行期间失效: ${workOrderId}, 可能存在并发风险`);
          // 记录告警，但不回滚（因为更新已完成）
          await this.logLockWarning(workOrderId, workerId, job.id, duration);
        }

        console.log(`✅ 工单更新任务完成: ${job.id}, WorkOrder: ${workOrderId}, 耗时: ${duration}ms, Worker: ${workerId}`);

        await job.progress(100);
        
        return {
          success: true,
          workOrderId,
          type,
          quantity,
          result,
          duration,
          workerId,
          lockValid, // 返回锁的有效性状态
          processedAt: new Date().toISOString()
        };
      } finally {
        if (extendLockInterval) {
          clearInterval(extendLockInterval);
        }
      }
      
    } catch (error) {
      const duration = Date.now() - startTime;
      console.error(`❌ 工单更新任务失败: ${job.id}, WorkOrder: ${workOrderId}, 耗时: ${duration}ms, Worker: ${workerId}`, error);

      await job.progress(100);
      
      throw new Error(`工单${workOrderId}更新失败: ${error.message}`);
    } finally {
      // 清理定时器
      if (extendLockInterval) {
        clearInterval(extendLockInterval);
      }
      
      // 释放锁
      if (lockAcquired) {
        await workOrderLockManager.releaseLock(workOrderId, workerId);
      }
    }
  });
}

// ============= 新增辅助方法 =============

/**
 * 记录锁失效告警
 */
static async logLockFailure(workOrderId, workerId, jobId) {
  try {
    const alertLog = {
      type: 'LOCK_FAILURE',
      workOrderId,
      workerId,
      jobId,
      timestamp: new Date(),
      message: '锁扩展失败超过阈值，任务已终止',
      severity: 'CRITICAL'
    };
    
    console.error('🚨 锁失效告警:', JSON.stringify(alertLog));
    
    // 可以发送到监控系统或写入专门的告警日志
    // await MonitoringService.sendAlert(alertLog);
  } catch (error) {
    console.error('记录锁失效告警失败:', error);
  }
}

/**
 * 记录锁有效性警告
 */
static async logLockWarning(workOrderId, workerId, jobId, duration) {
  try {
    const warningLog = {
      type: 'LOCK_WARNING',
      workOrderId,
      workerId,
      jobId,
      duration,
      timestamp: new Date(),
      message: '任务执行期间锁失效，可能存在并发风险',
      severity: 'WARNING'
    };
    
    console.warn('⚠️ 锁有效性警告:', JSON.stringify(warningLog));
    
    // 可以发送到监控系统
    // await MonitoringService.sendAlert(warningLog);
  } catch (error) {
    console.error('记录锁警告失败:', error);
  }
}
```

---

## 改进方案2: 实现幂等性机制

### 当前问题
如果锁失效导致重复执行，无法识别和阻止重复的更新请求。

### 数据模型

首先创建更新日志模型：

```javascript
// model/project/workOrderUpdateLog.js

const mongoose = require('mongoose');
const Schema = mongoose.Schema;

const workOrderUpdateLogSchema = new Schema({
  // 唯一请求ID（用于幂等性控制）
  requestId: {
    type: String,
    required: true,
    unique: true, // 唯一索引，防止重复
    index: true,
  },
  
  // 工单信息
  workOrderId: {
    type: Schema.Types.ObjectId,
    ref: 'production_plan_work_order',
    required: true,
    index: true,
  },
  workOrderNo: String,
  
  // 更新类型和数量
  type: {
    type: String,
    enum: ['input', 'output'],
    required: true,
  },
  quantity: {
    type: Number,
    required: true,
  },
  
  // 更新前后的值
  beforeValue: Number,
  afterValue: Number,
  
  // 处理信息
  workerId: String,
  jobId: String,
  processedAt: {
    type: Date,
    default: Date.now,
    index: true,
  },
  duration: Number, // 处理耗时（毫秒）
  
  // 上下文信息
  logContext: {
    relatedBarcode: String,
    barcodeOperation: String,
    operatorId: String,
    processStepId: String,
    processName: String,
    processCode: String,
    reason: String,
    remark: String,
    ipAddress: String,
    userAgent: String,
    source: String,
    isAutomatic: Boolean,
  },
  
  // 状态信息
  status: {
    type: String,
    enum: ['SUCCESS', 'FAILED', 'DUPLICATE'],
    default: 'SUCCESS',
  },
  errorMessage: String,
  
  // 数据一致性校验
  isVerified: {
    type: Boolean,
    default: false,
  },
  verifiedAt: Date,
}, {
  timestamps: true,
  collection: 'work_order_update_logs'
});

// 复合索引（优化查询性能）
workOrderUpdateLogSchema.index({ workOrderId: 1, processedAt: -1 });
workOrderUpdateLogSchema.index({ status: 1, processedAt: -1 });

// TTL索引（可选：30天后自动删除日志）
workOrderUpdateLogSchema.index({ processedAt: 1 }, { expireAfterSeconds: 30 * 24 * 60 * 60 });

module.exports = mongoose.model('work_order_update_log', workOrderUpdateLogSchema);
```

### 改进的执行方法

```javascript
// materialProcessFlowService.js
// 修改 _executeWorkOrderQuantityUpdate 方法

static async _executeWorkOrderQuantityUpdate(
  workOrderId,
  type,
  quantity = 1,
  logContext = {}
) {
  // 从logContext中提取requestId，如果没有则生成一个
  const requestId = logContext.requestId || 
    `${workOrderId}_${type}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  
  try {
    // ============= 幂等性检查 =============
    const WorkOrderUpdateLog = require('../model/project/workOrderUpdateLog');
    
    // 检查是否已经处理过这个请求
    const existingLog = await WorkOrderUpdateLog.findOne({ 
      requestId,
      status: 'SUCCESS' 
    });
    
    if (existingLog) {
      console.log(`🔁 请求已处理，跳过重复执行: ${requestId}`);
      console.log(`   原处理时间: ${existingLog.processedAt}`);
      console.log(`   原处理结果: ${existingLog.beforeValue} → ${existingLog.afterValue}`);
      
      // 记录重复请求
      await WorkOrderUpdateLog.create({
        requestId: `${requestId}_DUPLICATE_${Date.now()}`,
        workOrderId,
        type,
        quantity,
        status: 'DUPLICATE',
        workerId: logContext.workerId,
        jobId: logContext.jobId,
        logContext,
        errorMessage: `重复请求，原请求ID: ${requestId}`,
      });
      
      return {
        success: true,
        isDuplicate: true,
        originalLog: existingLog,
        message: '请求已处理过，跳过重复执行',
      };
    }
    
    // ============= 执行更新（保持原有逻辑） =============
    if (!workOrderId) {
      console.log("未提供工单ID，跳过更新工单数量");
      throw new Error("未提供工单ID");
    }

    const updateField = type === "input" ? "inputQuantity" : "outputQuantity";

    // 获取更新前的工单信息
    const beforeWorkOrder = await mongoose
      .model("production_plan_work_order")
      .findById(workOrderId)
      .populate("materialId");

    if (!beforeWorkOrder) {
      console.log(`未找到工单(ID: ${workOrderId})`);
      throw new Error(`未找到工单(ID: ${workOrderId})`);
    }

    // 记录更新前的值
    const beforeQuantity = beforeWorkOrder[updateField] || 0;
    const beforeStatus = beforeWorkOrder.status;

    // 扣减检查
    if (quantity < 0) {
      if (beforeQuantity + quantity < 0) {
        const requestedDecrease = -quantity;
        quantity = -beforeQuantity;
        console.log(
          `工单(ID: ${workOrderId})${
            type === "input" ? "投入" : "产出"
          }量不足，最多扣减到0（当前量：${beforeQuantity}，请求扣减：${requestedDecrease}，实际扣减：${-quantity}）`
        );
      }
    }

    // 准备更新数据
    const updateData = {
      $inc: { [updateField]: quantity },
      $set: {
        updateTime: new Date(),
      },
    };

    if (!beforeWorkOrder.createBy) {
      updateData.$set.createBy = beforeWorkOrder.updateBy;
    }

    // 计算进度和状态（保持原有逻辑）
    if (type === "output") {
      const newOutputQuantity = (beforeWorkOrder.outputQuantity || 0) + quantity;
      const planProductionQuantity = beforeWorkOrder.planProductionQuantity || 0;
      const scrapQuantity = beforeWorkOrder.scrapQuantity || 0;
      
      const totalTargetQuantity = planProductionQuantity + scrapQuantity;
      if (totalTargetQuantity > 0) {
        const newProgress = Math.min(100, Math.floor(
          (newOutputQuantity / totalTargetQuantity) * 100
        ));
        updateData.$set.progress = newProgress;
      }

      if (quantity < 0 && beforeWorkOrder.status === "COMPLETED") {
        updateData.$set.status = "PAUSED";
      }
      else if (newOutputQuantity >= planProductionQuantity) {
        updateData.$set.status = "COMPLETED";
        updateData.$set.endTime = new Date();
        updateData.$set.progress = 100;
      }
    }

    // ============= 原子更新操作 =============
    const startUpdateTime = Date.now();
    const workOrder = await mongoose
      .model("production_plan_work_order")
      .findOneAndUpdate(
        { _id: workOrderId },
        updateData,
        { 
          new: true,
          populate: "materialId"
        }
      );

    if (!workOrder) {
      throw new Error(`更新工单失败(ID: ${workOrderId})`);
    }

    const updateDuration = Date.now() - startUpdateTime;
    const afterQuantity = workOrder[updateField];

    // ============= 记录成功的更新日志 =============
    try {
      await WorkOrderUpdateLog.create({
        requestId,
        workOrderId: workOrder._id,
        workOrderNo: workOrder.workOrderNo || workOrder.workOrderNumber || "",
        type,
        quantity,
        beforeValue: beforeQuantity,
        afterValue: afterQuantity,
        workerId: logContext.workerId,
        jobId: logContext.jobId,
        duration: updateDuration,
        logContext,
        status: 'SUCCESS',
      });
      
      console.log(`📝 已记录更新日志: ${requestId}`);
    } catch (logError) {
      // 日志记录失败不应该影响主流程，但要记录错误
      console.error(`⚠️ 记录更新日志失败 (不影响主流程):`, logError);
      
      // 如果是重复键错误，说明存在并发问题
      if (logError.code === 11000) {
        console.error(`🚨 检测到并发重复更新: ${requestId}`);
        // 这里可以触发告警
      }
    }

    // 处理关联工单（保持原有逻辑）
    if (type === "output" && workOrder.status === "COMPLETED" && beforeStatus !== "COMPLETED") {
      console.log(`工单(ID: ${workOrderId})已完成 - 产出量: ${workOrder.outputQuantity}, 计划数量: ${workOrder.planProductionQuantity}`);
      await this.completeAllRelatedWorkOrders(workOrder._id);
    }

    console.log(`✅ 工单${type === "input" ? "投入" : "产出"}数量更新成功:`, {
      workOrderId: workOrder._id,
      workOrderNo: workOrder.workOrderNo,
      beforeQuantity,
      afterQuantity,
      change: quantity,
    });

    return {
      success: true,
      isDuplicate: false,
      workOrder,
      beforeQuantity,
      afterQuantity,
      requestId,
    };
    
  } catch (error) {
    // ============= 记录失败的日志 =============
    try {
      const WorkOrderUpdateLog = require('../model/project/workOrderUpdateLog');
      await WorkOrderUpdateLog.create({
        requestId: `${requestId}_FAILED_${Date.now()}`,
        workOrderId,
        type,
        quantity,
        status: 'FAILED',
        errorMessage: error.message,
        workerId: logContext.workerId,
        jobId: logContext.jobId,
        logContext,
      });
    } catch (logError) {
      console.error('记录失败日志时出错:', logError);
    }
    
    console.error(
      `更新工单${type === "input" ? "投入" : "产出"}数量失败:`,
      error
    );
    
    throw error;
  }
}
```

### 在队列中传递requestId

```javascript
// queueService.js 修改
static async addWorkOrderQuantityUpdate(workOrderId, type, quantity = 1, logContext = {}) {
  try {
    console.log(`添加工单更新任务到队列: ${workOrderId}, type: ${type}, quantity: ${quantity}`);

    // ============= 生成唯一的请求ID =============
    const requestId = `${workOrderId}_${type}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    const job = await workOrderQueue.add(
      'update-quantity',
      {
        workOrderId,
        type,
        quantity,
        logContext: {
          ...logContext,
          requestId, // 将requestId添加到logContext中
        },
        timestamp: Date.now(),
        requestId, // 同时作为顶层字段
      },
      {
        jobId: `${requestId}`, // 使用requestId作为jobId，确保唯一性
        priority: this.getTaskPriority(type),
        delay: 0,
      }
    );

    console.log(`工单更新任务已添加到队列: JobId=${job.id}, WorkOrderId=${workOrderId}, RequestId=${requestId}`);

    return {
      success: true,
      jobId: job.id,
      requestId, // 返回requestId
      workOrderId: workOrderId,
      type: type,
      quantity: quantity,
      message: '更新任务已加入队列',
      estimatedDelay: await this.getEstimatedDelay(),
      queueLength: (workOrderQueue.getWaiting ? 
        await workOrderQueue.getWaiting() : 
        await workOrderQueue.waiting()).length
    };

  } catch (error) {
    console.error('添加工单更新任务到队列失败:', error);
    return {
      success: false,
      error: error.message,
      workOrderId: workOrderId,
      type: type,
      quantity: quantity
    };
  }
}
```

---

## 改进方案3: 数据一致性监控

### 定期校验脚本

```javascript
// scripts/verify-workorder-consistency.js

const mongoose = require('mongoose');
require('../db'); // 连接数据库

const WorkOrder = require('../model/project/productionPlanWorkOrder');
const WorkOrderUpdateLog = require('../model/project/workOrderUpdateLog');

/**
 * 验证工单数据一致性
 */
async function verifyWorkOrderConsistency(options = {}) {
  const {
    workOrderId = null, // 指定工单ID，为null则检查所有
    days = 7, // 检查最近N天的数据
    autoFix = false, // 是否自动修复不一致
  } = options;

  console.log('=== 开始工单数据一致性校验 ===');
  console.log(`检查范围: 最近${days}天`);
  console.log(`自动修复: ${autoFix ? '是' : '否'}`);

  try {
    // 构建查询条件
    const dateThreshold = new Date();
    dateThreshold.setDate(dateThreshold.getDate() - days);

    const logQuery = {
      processedAt: { $gte: dateThreshold },
      status: 'SUCCESS',
    };

    if (workOrderId) {
      logQuery.workOrderId = workOrderId;
    }

    // 获取所有成功的更新日志
    const logs = await WorkOrderUpdateLog.find(logQuery).sort({ processedAt: 1 });

    console.log(`找到 ${logs.length} 条更新日志`);

    // 按工单ID分组
    const logsByWorkOrder = {};
    logs.forEach(log => {
      const id = log.workOrderId.toString();
      if (!logsByWorkOrder[id]) {
        logsByWorkOrder[id] = { input: [], output: [] };
      }
      logsByWorkOrder[id][log.type].push(log);
    });

    const results = {
      total: 0,
      consistent: 0,
      inconsistent: 0,
      details: [],
    };

    // 逐个工单进行校验
    for (const [workOrderIdStr, groupedLogs] of Object.entries(logsByWorkOrder)) {
      results.total++;

      // 获取当前工单数据
      const workOrder = await WorkOrder.findById(workOrderIdStr);
      
      if (!workOrder) {
        console.warn(`⚠️ 工单不存在: ${workOrderIdStr}`);
        continue;
      }

      // 计算日志中的投入量和产出量
      const calculateQuantityFromLogs = (logs) => {
        if (logs.length === 0) return 0;
        
        // 使用最后一条日志的afterValue作为最终值
        // 这样可以处理扣减的情况
        return logs[logs.length - 1].afterValue;
      };

      const expectedInput = calculateQuantityFromLogs(groupedLogs.input);
      const expectedOutput = calculateQuantityFromLogs(groupedLogs.output);

      const actualInput = workOrder.inputQuantity || 0;
      const actualOutput = workOrder.outputQuantity || 0;

      const inputMatch = expectedInput === actualInput;
      const outputMatch = expectedOutput === actualOutput;

      if (inputMatch && outputMatch) {
        results.consistent++;
        console.log(`✅ 工单 ${workOrder.workOrderNo || workOrderIdStr}: 数据一致`);
      } else {
        results.inconsistent++;
        
        const detail = {
          workOrderId: workOrderIdStr,
          workOrderNo: workOrder.workOrderNo,
          inputMatch,
          outputMatch,
          input: {
            expected: expectedInput,
            actual: actualInput,
            diff: actualInput - expectedInput,
          },
          output: {
            expected: expectedOutput,
            actual: actualOutput,
            diff: actualOutput - expectedOutput,
          },
          logs: {
            inputCount: groupedLogs.input.length,
            outputCount: groupedLogs.output.length,
          },
        };

        results.details.push(detail);

        console.error(`❌ 工单 ${workOrder.workOrderNo || workOrderIdStr}: 数据不一致`);
        console.error(`   投入量 - 期望: ${expectedInput}, 实际: ${actualInput}, 差异: ${detail.input.diff}`);
        console.error(`   产出量 - 期望: ${expectedOutput}, 实际: ${actualOutput}, 差异: ${detail.output.diff}`);

        // 自动修复（谨慎使用）
        if (autoFix) {
          console.log(`🔧 自动修复工单: ${workOrderIdStr}`);
          
          await WorkOrder.findByIdAndUpdate(workOrderIdStr, {
            $set: {
              inputQuantity: expectedInput,
              outputQuantity: expectedOutput,
              updateTime: new Date(),
            }
          });

          console.log(`✅ 已修复工单数据`);
        }
      }
    }

    // 输出统计结果
    console.log('\n=== 校验结果统计 ===');
    console.log(`总工单数: ${results.total}`);
    console.log(`一致: ${results.consistent} (${((results.consistent / results.total) * 100).toFixed(2)}%)`);
    console.log(`不一致: ${results.inconsistent} (${((results.inconsistent / results.total) * 100).toFixed(2)}%)`);

    if (results.inconsistent > 0) {
      console.log('\n=== 不一致详情 ===');
      results.details.forEach(detail => {
        console.log(`\n工单: ${detail.workOrderNo || detail.workOrderId}`);
        console.log(`  投入量: 期望 ${detail.input.expected}, 实际 ${detail.input.actual}, 差异 ${detail.input.diff}`);
        console.log(`  产出量: 期望 ${detail.output.expected}, 实际 ${detail.output.actual}, 差异 ${detail.output.diff}`);
      });
    }

    return results;

  } catch (error) {
    console.error('校验失败:', error);
    throw error;
  }
}

// 命令行使用
if (require.main === module) {
  const workOrderId = process.argv[2] || null;
  const days = parseInt(process.argv[3]) || 7;
  const autoFix = process.argv[4] === 'true';

  verifyWorkOrderConsistency({
    workOrderId,
    days,
    autoFix,
  })
    .then(() => {
      console.log('\n校验完成');
      process.exit(0);
    })
    .catch(error => {
      console.error('校验失败:', error);
      process.exit(1);
    });
}

module.exports = { verifyWorkOrderConsistency };
```

### 定时任务配置

```javascript
// 在app.js或单独的调度文件中添加

const schedule = require('node-schedule');
const { verifyWorkOrderConsistency } = require('./scripts/verify-workorder-consistency');

// 每天凌晨3点执行数据一致性校验
schedule.scheduleJob('0 3 * * *', async () => {
  console.log('开始定时工单数据一致性校验');
  
  try {
    const results = await verifyWorkOrderConsistency({
      days: 1, // 检查最近1天的数据
      autoFix: false, // 不自动修复，只告警
    });

    // 如果发现不一致，发送告警
    if (results.inconsistent > 0) {
      // 这里可以集成邮件、钉钉、企业微信等告警
      console.error(`🚨 发现 ${results.inconsistent} 个工单数据不一致，请及时处理！`);
      
      // 示例：发送钉钉告警
      // await sendDingTalkAlert({
      //   title: '工单数据不一致告警',
      //   content: `发现 ${results.inconsistent} 个工单数据不一致`,
      //   details: results.details,
      // });
    }
  } catch (error) {
    console.error('定时校验失败:', error);
  }
});
```

---

## 部署步骤

### 1. 数据库迁移

```bash
# 创建更新日志集合和索引
mongo your_database_name

db.createCollection("work_order_update_logs")

db.work_order_update_logs.createIndex({ "requestId": 1 }, { unique: true })
db.work_order_update_logs.createIndex({ "workOrderId": 1, "processedAt": -1 })
db.work_order_update_logs.createIndex({ "status": 1, "processedAt": -1 })
db.work_order_update_logs.createIndex({ "processedAt": 1 }, { expireAfterSeconds: 2592000 })
```

### 2. 代码部署

```bash
# 1. 备份当前代码
cp dcMes_server/services/queueService.js dcMes_server/services/queueService.js.backup
cp dcMes_server/services/materialProcessFlowService.js dcMes_server/services/materialProcessFlowService.js.backup

# 2. 更新代码（应用上述改进）

# 3. 重启服务
pm2 restart all
```

### 3. 监控验证

```bash
# 观察日志，确认改进生效
pm2 logs | grep "锁扩展"

# 运行一致性校验
node dcMes_server/scripts/verify-workorder-consistency.js null 7 false
```

---

## 监控指标

### 在代码中添加监控指标

```javascript
// queueService.js 添加监控统计

class QueueMetrics {
  constructor() {
    this.metrics = {
      // 锁相关指标
      lockExtensionAttempts: 0,
      lockExtensionSuccesses: 0,
      lockExtensionFailures: 0,
      lockInvalidations: 0,
      
      // 任务相关指标
      tasksProcessed: 0,
      tasksSucceeded: 0,
      tasksFailed: 0,
      
      // 幂等性指标
      duplicateRequests: 0,
      
      // 性能指标
      totalDuration: 0,
      maxDuration: 0,
      minDuration: Infinity,
    };
  }

  recordLockExtension(success) {
    this.metrics.lockExtensionAttempts++;
    if (success) {
      this.metrics.lockExtensionSuccesses++;
    } else {
      this.metrics.lockExtensionFailures++;
    }
  }

  recordLockInvalidation() {
    this.metrics.lockInvalidations++;
  }

  recordTask(success, duration, isDuplicate = false) {
    this.metrics.tasksProcessed++;
    if (success) {
      this.metrics.tasksSucceeded++;
    } else {
      this.metrics.tasksFailed++;
    }
    
    if (isDuplicate) {
      this.metrics.duplicateRequests++;
    }

    this.metrics.totalDuration += duration;
    this.metrics.maxDuration = Math.max(this.metrics.maxDuration, duration);
    this.metrics.minDuration = Math.min(this.metrics.minDuration, duration);
  }

  getStats() {
    const avgDuration = this.metrics.tasksProcessed > 0
      ? this.metrics.totalDuration / this.metrics.tasksProcessed
      : 0;

    return {
      ...this.metrics,
      avgDuration,
      lockExtensionSuccessRate: this.metrics.lockExtensionAttempts > 0
        ? (this.metrics.lockExtensionSuccesses / this.metrics.lockExtensionAttempts * 100).toFixed(2) + '%'
        : 'N/A',
      taskSuccessRate: this.metrics.tasksProcessed > 0
        ? (this.metrics.tasksSucceeded / this.metrics.tasksProcessed * 100).toFixed(2) + '%'
        : 'N/A',
    };
  }

  reset() {
    this.metrics = {
      lockExtensionAttempts: 0,
      lockExtensionSuccesses: 0,
      lockExtensionFailures: 0,
      lockInvalidations: 0,
      tasksProcessed: 0,
      tasksSucceeded: 0,
      tasksFailed: 0,
      duplicateRequests: 0,
      totalDuration: 0,
      maxDuration: 0,
      minDuration: Infinity,
    };
  }
}

const queueMetrics = new QueueMetrics();

// 每分钟输出一次统计
setInterval(() => {
  const stats = queueMetrics.getStats();
  console.log('=== 队列服务统计 (最近1分钟) ===');
  console.log(JSON.stringify(stats, null, 2));
  
  // 检测异常情况并告警
  if (stats.lockInvalidations > 0) {
    console.error(`🚨 检测到 ${stats.lockInvalidations} 次锁失效事件！`);
  }
  
  if (stats.duplicateRequests > 0) {
    console.warn(`⚠️ 检测到 ${stats.duplicateRequests} 次重复请求！`);
  }
  
  // 重置计数器（或者保留累计值，根据需求决定）
  // queueMetrics.reset();
}, 60000);
```

---

## 测试验证

### 并发测试脚本

```javascript
// test/workorder-concurrent-test.js

const axios = require('axios');

async function testConcurrentUpdates() {
  const workOrderId = '你的测试工单ID';
  const concurrentRequests = 10;

  console.log(`发起 ${concurrentRequests} 个并发请求...`);

  const requests = [];
  for (let i = 0; i < concurrentRequests; i++) {
    requests.push(
      axios.post('http://localhost:3000/api/v1/material-process-flow/update-workorder', {
        workOrderId,
        type: 'input',
        quantity: 1,
        logContext: {
          operatorId: 'TEST',
          source: 'CONCURRENT_TEST',
        },
      })
    );
  }

  const results = await Promise.allSettled(requests);

  console.log('\n测试结果:');
  console.log(`成功: ${results.filter(r => r.status === 'fulfilled').length}`);
  console.log(`失败: ${results.filter(r => r.status === 'rejected').length}`);

  // 查询最终的投入量
  const workOrder = await axios.get(`http://localhost:3000/api/v1/workorder/${workOrderId}`);
  console.log(`\n最终投入量: ${workOrder.data.inputQuantity}`);
  console.log(`预期投入量: ${concurrentRequests}`);
  console.log(`是否一致: ${workOrder.data.inputQuantity === concurrentRequests ? '✅' : '❌'}`);
}

testConcurrentUpdates().catch(console.error);
```

---

## 总结

这套改进方案实现了：

1. ✅ **锁续期监控**: 实时检测锁的有效性，失败时立即终止任务
2. ✅ **幂等性保护**: 基于requestId防止重复执行
3. ✅ **数据一致性校验**: 定期验证数据完整性
4. ✅ **监控告警**: 完整的指标统计和异常告警
5. ✅ **降级处理**: 在异常情况下的安全处理机制

**预期效果**:
- 完全解决网络延迟导致的重复扣减问题
- 提供数据不一致的检测和修复能力
- 增强系统的可观测性和可维护性

**建议实施顺序**:
1. 第一周: 实施方案1（锁续期监控） - 最关键
2. 第二周: 实施方案2（幂等性机制） - 次关键
3. 第三周: 实施方案3（数据一致性监控） - 辅助工具










