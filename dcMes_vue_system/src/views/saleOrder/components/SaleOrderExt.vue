    ...
    >
      ...
      <el-card class="box-card" shadow="hover">
        ...
        <el-row :gutter="20">
          <!-- 产品型号 -->
          <el-col :span="8">
            <el-form-item label="产品型号" prop="FProductModel">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="托盘标贴及包装方式" prop="FBoxLabelPackage">
              <el-input
                type="textarea"
                v-model="formData.FBoxLabelPackage"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        FSaleCountry: [
          { required: true, message: "请输入销售国家地区", trigger: "blur" },
        ],
        FMinSpec: [{ required: true, message: "请输入铭牌", trigger: "blur" }],
        FVoltageRatio: [
          { required: true, message: "请输入电压功率", trigger: "blur" },
        ],
        FBatterySpec: [
        ...
        FPrintColor: [
          { required: true, message: "请输入印刷颜色", trigger: "blur" },
        ],
        FManual: [
          { required: true, message: "请输入说明书要求", trigger: "blur" },
        ],
        FSelfSeal: [
          { required: true, message: "请输入自封袋要求", trigger: "blur" },
        ],
        FCertification: [
          { required: true, message: "请输入认证要求", trigger: "blur" },
        ],
        FProductColor: [
          { required: true, message: "请输入产品颜色", trigger: "blur" },
        ],
        FColorLabel: [
          { required: true, message: "请输入彩盒标贴要求", trigger: "blur" },
        ],
        FTrademark: [
          { required: true, message: "请输入商标要求", trigger: "blur" },
        ],
        FUDI: [{ required: true, message: "请输入UDI", trigger: "blur" }],
        FUDISerialNoRule: [
          { required: true, message: "请输入序列号规则", trigger: "blur" },
        ],
        FUDIType: [{ required: true, message: "请输入形制", trigger: "blur" }],
        FPEBagReq: [
          { required: true, message: "请输入PE袋要求", trigger: "blur" },
        ],
        FBoxLabel: [
          { required: true, message: "请输入托盘标贴要求", trigger: "blur" },
        ],
        FBoxLabelPackage: [
          {
            required: true,
            message: "请输入托盘标贴及包装方式",
            trigger: "blur",
          },
        ],
      },
      ...
      try {
        ...
        if (result.code === 200 && result.data.length > 0) {
          ...
          if (this.formData.FColorLabelImage) {
            this.colorLabelImageList = [
              { url: this.formData.FColorLabelImage },
            ];
          }
        ...
        if (valid) {
          try {
            if (this.formData._id) {
              // 更新
              await updateData("k3_SAL_SaleOrderExt", {
                query: { FSaleOrderId: this.saleOrderId },
