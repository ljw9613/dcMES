...
**需求描述：**
在 `dcMes_vue_system/src/views/scanBarCode/index.vue` 和 `dcMes_vue_system/src/views/scanBarCodeConver/index.vue` 中，关键物料条码原先主要在提交阶段才由后端统一拦截重复，现场扫码时反馈偏晚。

**本次优化：**
- 在前端扫码阶段新增关键物料本地重复判断
- 扫入关键物料时，立即比对当前页面已扫描的关键物料条码
- 若发现重复，直接弹出错误提示并阻止继续填入当前槽位
- 保留后端提交阶段的重复校验，作为最终兜底
