                    ...
                    <el-col :span="12">
                        <el-form-item label="生产订单号" prop="productionOrderId">
                            <zr-select v-model="formData.productionOrderId" collection="k3_PRD_MO"
                                :search-fields="['FBillNo', 'FMaterialName']" label-key="FBillNo" sub-key="FBillNo"
                                :multiple="false" placeholder="请输入生产单号/物料名称搜索" clearable style="width: 100%"
    ...
    methods: {
        ...
        async handleSubmit() {
            try {
                ...
                let saveResult
                // 确保 formData 中包含 FRequisitionBillId
                const saveData = {
                    ...this.formData,
                    FRequisitionBillId: this.orderData._id,
                ...
                if (this.extId) {
                    ...
                    // 新增数据
                    saveResult = await addData('k3_PUR_RequisitionBillExt',
                        saveData
                    )
                }
        ...
        async fetchExtData() {
            try {
                ...
                const res = await getData('K3_PUR_RequisitionBillExt', req)
