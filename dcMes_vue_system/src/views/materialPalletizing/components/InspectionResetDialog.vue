<template>
  <el-dialog title="托盘抽检复位" :visible.sync="dialogVisible" width="50%" @closed="handleClose">
    <div v-if="palletData">
      <el-descriptions title="托盘信息" :column="2" border>
        <el-descriptions-item label="托盘编号">{{ palletData.palletCode }}</el-descriptions-item>
        ...
        <el-descriptions-item label="物料信息">
          ...
          <el-tag size="mini" v-if="palletData.materialSpec">{{ palletData.materialSpec }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="待复位数量">
          <el-progress :percentage="progressPercentage" :format="formatProgress"></el-progress>
        </el-descriptions-item>
      </el-descriptions>
      ...
      <el-form :model="scanForm" ref="scanForm" label-width="100px">
        <el-form-item label="扫描条码" prop="barcode" :rules="[{ required: true, message: '请输入条码', trigger: 'blur' }]">
          <el-input 
        ...
        <el-form-item>
          <el-button type="primary" @click="handleScan" :loading="scanning">校验条码</el-button>
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="handleSubmit" :disabled="!isCompleted">确认完成</el-button>
    </span>
  </el-dialog>
</template>
...
export default {
  ...
  computed: {
    progressPercentage() {
      if (!this.palletData || !this.palletData.palletBarcodes || !this.palletData.palletBarcodes.length) {
        return 0;
      }
      
      const inspectingCount = this.palletData.palletBarcodes.filter(
      ...
      const inspectedCount = this.palletData.palletBarcodes.filter(
        ...
        this.barcodesTableData.some(b => b.barcode === item.barcode)
      ).length;
      
      return inspectingCount === 0 ? 100 : Math.floor((inspectedCount / inspectingCount) * 100);
    },
    isCompleted() {
      return this.progressPercentage === 100;
    },
    canCompleteAll() {
      return this.barcodesTableData && 
             this.barcodesTableData.length > 0 && 
             this.barcodesTableData.some(item => item.inspectionStatus === 'INSPECTING');
    }
  },
  watch: {
    visible(val) {
  ...
  methods: {
    ...
    formatProgress(percentage) {
      ...
      return `${inspectedCount}/${inspectingCount}`;
    },
    ...
    async handleScan() {
      ...
      try {
        ...
        if (palletBarcodeIndex !== -1) {
          this.palletData.palletBarcodes[palletBarcodeIndex].inspectionStatus = "INSPECTED";
