                    ...
                    <el-form :model="scanForm" ref="scanForm" label-width="100%">
                        ...
                        <div class="material-section" v-if="processMaterials.length > 0">
                            <el-form-item v-for="material in processMaterials" :key="material._id"
                                :label="`编号：${material.materialCode}  名称：${material.materialName}`" label-width="100%"
                                class="vertical-form-item">
                                <div class="input-with-status">
    ...
    methods: {
            ...
            try {
                ...
                if (response.code === 200 && response.data) {
                    ...
                    if (materialId && processStepId && processStepId.processName) {
                        ...
                        // 更新生产计划ID缓存
                        localStorage.setItem('lastWorkProductionPlanWorkOrderId_batchNew',
        ...
        async handleSave() {
            ...
            try {
                ...
                // 更新生产计划ID缓存
                localStorage.setItem('lastWorkProductionPlanWorkOrderId_batchNew', this.formData.workProductionPlanWorkOrderId || '');
            ...
            try {
                ...
                try {
                    ...
                    if (processMaterialsResponse.data) {
                        ...
                        const storedPlanId = localStorage.getItem('lastWorkProductionPlanWorkOrderId_batchNew');
                        ...
                        if (currentPlanId && currentPlanId !== storedPlanId) {
                            ...
                            // 更新存储的生产计划ID
                            localStorage.setItem('lastWorkProductionPlanWorkOrderId_batchNew', currentPlanId || '');
                        }
            ...
            try {
                    ...
                    this.$notify({
                        ...
                        message: `
                                <div style="line-height: 1.5">
                                    ...
                                    <div>物料编码: ${this.mainMaterialCode}</div>
                ...
                for (const material of this.processMaterials) {
                    if (material.materialCode === materialCode) {
                        ...
                        // // 检查是否已扫描
                        // if (this.validateStatus[material._id]) {
                        //     this.$message.warning('该子物料已扫描');
                        //     this.popupType = 'ng';
                        //     this.showPopup = true;
                        //     tone(cfbd);
                        //     return;
                ...
                if (this.mainMaterialId && this.palletForm.productionOrderId) {
                    const palletResponse = await getData('material_palletizing', {
                        query: {
                            ...
                            productLineId: this.productLineId, status: 'STACKING', materialId: this.mainMaterialId
                        },
