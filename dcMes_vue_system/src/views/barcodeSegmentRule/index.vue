  ...
  <div class="app-container">
      ...
      <el-form :model="searchForm" ref="searchForm" class="demo-form-inline">
        ...
        <el-form-item>
          ...
          <el-button
            ...
            icon="el-icon-plus"
            @click="handleCreate"
            v-if="$checkPermission('条码生成规则新增规则')"
            >新增规则</el-button
          >
          <el-button
            type="danger"
            ...
            v-if="$checkPermission('条码生成规则批量删除')"
            >批量删除</el-button
          >
        </el-form-item>
      </el-form>
    ...
    >
      ...
      >
        ...
        <el-form-item label="总长度" prop="totalLength">
          <el-input-number
            v-model="currentRule.totalLength"
            :min="1"
            :precision="0"
            :controls="true"
        ...
        <div
          v-for="(segment, index) in currentRule.segments"
          :key="index"
          class="segment-item"
        >
          <el-card class="segment-card">
            <div slot="header" class="clearfix">
              ...
              <div style="float: right">
                ...
                <el-button
                  type="text"
                  icon="el-icon-bottom"
                  :disabled="index === currentRule.segments.length - 1"
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item
                  :label="'前缀'"
                  :prop="'segments.' + index + '.config.prefix'"
                >
                  <el-input
                    v-model="segment.config.prefix"
                    placeholder="如: (01)"
                  >
                    <template slot="append">
                      <el-checkbox v-model="segment.config.showPrefix"
                        >是否打印</el-checkbox
                      >
                    </template>
                  </el-input>
              ...
              <el-col :span="12">
                <el-form-item
                  :label="'后缀'"
                  :prop="'segments.' + index + '.config.suffix'"
                >
                  <el-input
                    v-model="segment.config.suffix"
                    placeholder="如: (H1)"
                  >
                    <template slot="append">
                      <el-checkbox v-model="segment.config.showSuffix"
                        >是否打印</el-checkbox
                      >
                    </template>
                  </el-input>
            ...
            <template v-if="segment.type === 'date'">
              <el-form-item
                :label="'日期格式'"
                :prop="'segments.' + index + '.config.dateFormat'"
              >
                <el-input
                  v-model="segment.config.dateFormat"
              ...
              <el-form-item label="年份映射">
                <el-button
                  size="small"
                  type="primary"
                  @click="addMapping(segment.config.yearMappings)"
                  >添加映射</el-button
                >
                <el-table
                  :data="segment.config.yearMappings"
                  border
              ...
              <el-form-item label="月份映射">
                <el-button
                  size="small"
                  type="primary"
                  @click="addMapping(segment.config.monthMappings)"
                  >添加映射</el-button
                >
                <el-table
                  :data="segment.config.monthMappings"
                  border
                  style="margin-top: 10px"
                >
              ...
              <el-form-item label="日期映射">
                <el-button
                  size="small"
                  type="primary"
                  @click="addMapping(segment.config.dayMappings)"
                  >添加映射</el-button
                >
                <el-table
                  :data="segment.config.dayMappings"
                  border
                  style="margin-top: 10px"
                >
            ...
            <template v-if="segment.type === 'sequence'">
              <el-row :gutter="20">
                <el-col :span="8">
                  <el-form-item
                    :label="'起始值'"
                    :prop="'segments.' + index + '.config.startValue'"
                  >
                    <el-input-number
                      v-model="segment.config.startValue"
                      :min="1"
                    />
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item
                    :label="'长度'"
                    :prop="'segments.' + index + '.config.length'"
                  >
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item
                    :label="'填充字符'"
                    :prop="'segments.' + index + '.config.padChar'"
                  >
                    <el-input
                      v-model="segment.config.padChar"
                      maxlength="1"
                      placeholder="如：0"
              ...
              <el-form-item label="数字映射">
                <div class="mapping-tip">
                  注意：映射规则将应用于填充后的序列号，包括填充字符
                </div>
                <el-button
                  size="small"
                  type="primary"
                  @click="addNumberMapping(segment.config)"
                  >添加映射</el-button
                >
                <el-table
                  :data="segment.config.numberMappings"
                  border
                  style="margin-top: 10px"
                >
            ...
            <template v-if="segment.type === 'constant'">
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item
                    :label="'固定值'"
                    :prop="'segments.' + index + '.config.constantValue'"
                  >
                    <el-input
                      v-model="segment.config.constantValue"
                      placeholder="如: 01"
                    />
                  </el-form-item>
                </el-col>
            ...
            <template v-if="segment.type === 'fieldMapping'">
              <el-form-item
                :label="'映射字段'"
                :prop="'segments.' + index + '.config.mappingField'"
              >
                <el-select
                  v-model="segment.config.mappingField"
                  style="width: 100%"
                >
              ...
              <el-form-item label="映射规则">
                <el-button
                  size="small"
                  type="primary"
                  @click="addMapping(segment.config.fieldMappings)"
                  >添加映射</el-button
                >
                <el-table
                  :data="segment.config.fieldMappings || []"
                  border
                  style="margin-top: 10px"
                >
          ...
          <el-card class="test-params-card">
            <div slot="header" class="clearfix">
              ...
              <el-button-group style="float: right">
                <el-button type="primary" size="small" @click="testGenerate"
                  >测试生成</el-button
                >
                ...
                <div class="param-section">
                  ...
                  <el-row :gutter="20">
                    <template v-for="(segment, index) in currentRule.segments">
                      <el-col
                        :span="12"
                        v-if="segment.type === 'fieldMapping'"
                        :key="'field-' + index"
                      >
                        <el-form-item :label="segment.name">
                          <el-input
                            v-model="
                              testParams.fieldValues[
          ...
          <el-card class="test-result-card" v-if="testResult">
            ...
            <div class="test-result-content">
              <div class="barcode-group">
                <div class="barcode-item">
                  ...
                  <div class="barcode-preview">
                    {{ testResult.displayBarcode }}
                  </div>
                </div>
                <div class="barcode-item">
                  <div class="barcode-label">基础打印条码:</div>
                  <div class="barcode-preview">
                    {{ testResult.printBarcode }}
                  </div>
                </div>
                ...
                <div
                  class="barcode-item"
                  v-if="testResult.transformedPrintBarcode"
                >
                  <div class="barcode-label">转换打印条码:</div>
                  <div class="barcode-preview">
                    {{ testResult.transformedPrintBarcode }}
                  </div>
                </div>
              </div>
              ...
              <div class="segment-breakdown">
                <el-row
                  v-for="(segment, index) in segmentBreakdown"
                  :key="index"
                  class="segment-detail"
                >
                  <el-col :span="6" class="segment-label"
                    >{{ segment.name }}:</el-col
                  >
                  <el-col :span="18">
                    ...
                    <div
                      class="segment-info"
                      v-if="segment.config.prefix || segment.config.suffix"
                    >
                      <small>
    ...
    <el-dialog title="绑定物料" :visible.sync="bindDialogVisible" width="70%">
      <div class="bind-material-dialog">
        ...
        <!-- 新增物料绑定 -->
        <el-card
          class="add-material-card"
          v-if="$checkPermission('条码生成规则绑定物料-新增物料绑定')"
        >
          <div slot="header">
            <span>新增物料绑定</span>
          </div>
...
export default {
  ...
  computed: {
    hasDateSegment() {
      return this.currentRule.segments.some(
        (segment) => segment.type === "date"
      );
    },
    hasFieldMapping() {
      return this.currentRule.segments.some(
  ...
  methods: {
      ...
      try {
        ...
        const formattedSegments = this.currentRule.segments.map((segment) => ({
          ...
          config: {
            ...
            showPrefix:
              segment.config.showPrefix !== undefined
                ? segment.config.showPrefix
                : true,
            showSuffix:
              segment.config.showSuffix !== undefined
                ? segment.config.showSuffix
                : true,
            constantValue:
              segment.type === "constant"
                ? segment.config.constantValue || ""
                : "",
            mappingField:
              segment.type === "fieldMapping"
                ? segment.config.mappingField || ""
            ...
            dateFormat:
              segment.type === "date"
                ? segment.config.dateFormat || "YYYY"
                : "",
            yearMappings:
              segment.type === "date" ? segment.config.yearMappings || [] : [],
            monthMappings:
              segment.type === "date" ? segment.config.monthMappings || [] : [],
            dayMappings:
              segment.type === "date" ? segment.config.dayMappings || [] : [],
            startValue:
              segment.type === "sequence" ? segment.config.startValue || 1 : 1,
            length:
              segment.type === "sequence" ? segment.config.length || 1 : 1,
            padChar:
              segment.type === "sequence" ? segment.config.padChar || "0" : "0",
            numberMappings:
            ...
            enableTransform:
              segment.type === "constant"
                ? segment.config.enableTransform || false
                : false,
            transformValue:
              segment.type === "constant"
                ? segment.config.transformValue || ""
                : "",
          },
        }));
    ...
    async generateBarcode(rule, params) {
      try {
        ...
        for (const segment of rule.segments) {
          const segmentResult = await this.generateSegmentValue(
            segment,
            params
          );
    ...
    handlePadCharChange(segment) {
      if (
        segment.config.numberMappings &&
        segment.config.numberMappings.length
      ) {
    ...
    async handleBatchBind() {
      if (
        ...
        this.materialForm.materialId.length === 0
      ) {
      ...
      try {
        ...
        const existingMaterials = this.boundMaterials.map(
          (item) => item.materialId
        );
        const duplicates = selectedMaterials.filter((id) =>
          existingMaterials.includes(id)
        );
        ...
        if (existingBindings.data && existingBindings.data.length > 0) {
          const boundMaterialInfo = existingBindings.data.map(
            (binding) =>
    ...
    async unbindMaterial(row) {
      try {
        ...
        await removeData("barcodeSegmentRuleMaterial", {
          query: { _id: row._id },
        });
