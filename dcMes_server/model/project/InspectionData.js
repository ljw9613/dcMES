...
const inspectionDataSchema = new mongoose.Schema(
  {
    ...
    dataUploadCountExceededLimit: {
      type: Boolean,
      description: "数据上传次数超出限制",
    },
...
inspectionDataSchema.index({ updateTime: -1 }); // 用于时间范围查询

// 复合索引：优化常见查询组合
// 扫描码 + 更新时间（用于扫描码查询时按时间排序，历史数据查询）
inspectionDataSchema.index({ scanCode: 1, updateTime: -1 });
// 设备 + 更新时间（用于设备筛选时按时间排序）
inspectionDataSchema.index({ machineId: 1, updateTime: -1 });
// 工序 + 更新时间（用于工序筛选时按时间排序）
inspectionDataSchema.index({ processId: 1, updateTime: -1 });
// 更新时间（用于时间范围查询和排序）
inspectionDataSchema.index({ updateTime: -1, _id: -1 });
