...
router.post("/api/v1/tripletData", async (req, res) => {
  try {
    ...
    const flowRecord = await MaterialProcessFlow.findOne({ barcode: snTrim })
      ...
      .lean();
    if (flowRecord && flowRecord.productionPlanWorkOrderId) {
      workOrderFields.productionPlanWorkOrderId = flowRecord.productionPlanWorkOrderId;
      ...
      if (workOrder && workOrder.workOrderNo) {
        workOrderFields.workOrderNo = workOrder.workOrderNo;
      }
    }
    ...
    return sendSuccess(
      res,
      {
        ...
        workOrderNo: updated.workOrderNo || null,
      },
      "绑定成功"
    );
  } catch (e) {
