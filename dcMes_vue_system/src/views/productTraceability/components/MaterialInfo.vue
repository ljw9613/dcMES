      ...
      <el-table-column label="关联物料">
        <template slot-scope="scope">
          <el-table
            ...
            :data="
              scope.row.children.filter((item) => item.nodeType === 'MATERIAL')
            "
            border
            class="inner-table"
          >
            ...
            <el-table-column label="类型" width="100">
              <template slot-scope="innerScope">
                ...
                <span
                  v-if="
                    !innerScope.row.isComponent && !innerScope.row.isKeyMaterial
                  "
                  >-</span
                >
              </template>
...
export default {
  ...
  computed: {
    processStepData() {
      return this.flowChartData[0] && this.flowChartData[0].children
        ? this.flowChartData[0].children.filter(
  ...
  methods: {
    ...
    async handleUnbind(row) {
      try {
        // 如果产品状态为已完成，显示额外确认
        if (this.productStatus === 'COMPLETED') {
          try {
            await this.$confirm(
              '此产品为"已完成"状态，确定需要进行解绑操作?解绑后绑定数据将会删除并无法恢复，可能会影响产品状态，确认要继续吗?',
              '状态提示',
              {
    ...
    handleReplaceComponent(row) {
      ...
      if (this.productStatus === 'COMPLETED') {
        this.$confirm(
          '此产品为"已完成"状态，确定需要进行组件替换?替换后可能会影响产品状态，确认要继续吗?',
          '警告',
          {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
