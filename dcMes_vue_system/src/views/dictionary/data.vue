...
export default {
  ...
  methods: {
    ...
    /** 查询字典类型详细 */
    getType(dictType) {
      getData('dictType', {query: {dictType: dictType}}).then(response => {
    ...
    /** 查询字典类型列表 */
    getTypeList() {
      getData('dictType', {query: this.queryParams}).then(response => {
    ...
    handleUpdate(row) {
      ...
      getRow('dictData', {query: {dictType: dictType, dictValue: row.dictValue}}).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form._id != undefined) {
            updateData('dictData', {query: {_id: this.form._id}, update: {...this.form}}).then(response => {
            ...
            addData('dictData', this.form).then(response => {
              this.$store.dispatch('dict/removeDict', this.queryParams.dictType);
    ...
    handleDelete(row) {
      this.$modal.confirm('是否确认删除字典标签为"' + row.dictLabel + '"的数据项？').then(function () {
        return removeData('dictData', {query: {_id: row._id}});
      }).then(() => {
        this.getList();
