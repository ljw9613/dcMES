...
class QueueService {
  ...
  static initializeWorkOrderProcessor() {
    ...
    // 设置并发数为1，确保严格的顺序处理
    workOrderQueue.process('update-quantity', 1, async (job) => {
      const startTime = Date.now();
      const { workOrderId, type, quantity, logContext, requestId } = job.data;

      try {
        console.log(`📝 开始处理工单更新任务: ${job.id}, WorkOrder: ${workOrderId}, Type: ${type}, Quantity: ${quantity}`);
        ...
        try {
          ...
          console.log(`✅ 工单更新任务完成: ${job.id}, WorkOrder: ${workOrderId}, 耗时: ${duration}ms`);
        ...
        const duration = Date.now() - startTime;
        console.error(`❌ 工单更新任务失败: ${job.id}, WorkOrder: ${workOrderId}, 耗时: ${duration}ms`, error);
  ...
  static initializePalletProcessor() {
    ...
    // 设置并发数为2，允许少量并发处理托盘任务
    palletQueue.process('handle-pallet-barcode', 2, async (job) => {
      const startTime = Date.now();
      const {
        lineId,
        lineName,
      ...
      try {
        console.log(`🎯 开始处理托盘任务: ${job.id}, 条码: ${mainBarcode}, 产线: ${lineName}`);
        ...
        const duration = Date.now() - startTime;
        console.log(`✅ 托盘处理任务完成: ${job.id}, 条码: ${mainBarcode}, 耗时: ${duration}ms`);
        ...
        const duration = Date.now() - startTime;
        console.error(`❌ 托盘处理任务失败: ${job.id}, 条码: ${mainBarcode}, 耗时: ${duration}ms`, error);
