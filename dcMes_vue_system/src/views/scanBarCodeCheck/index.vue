<template>
  <div class="scan-barcode-container">
    ...
    <div class="scan-input-area">
      <div class="input-wrapper">
        <el-input
          ref="barcodeInput"
          v-model="barcode"
          :placeholder="$t('scanBarCodeCheck.scanning.scanPlaceholder')"
          class="barcode-input"
          @keyup.enter.native="checkBarcode"
          @focus="handleFocus"
        >
          <i slot="prefix" class="el-icon-search"></i>
          <el-button slot="suffix" type="primary" @click="checkBarcode">{{
            $t("scanBarCodeCheck.scanning.query")
          }}</el-button>
        </el-input>
      </div>
    ...
    <div class="operation-buttons">
      <el-button type="primary" @click="handleScan" icon="el-icon-camera">
    ...
    <div v-if="isLoading" class="loading-container">
      <div
        v-loading="true"
        :element-loading-text="$t('scanBarCodeCheck.messages.queryingBarcode')"
      ></div>
    </div>
    ...
    <div v-if="scanResult && !isLoading" class="result-container">
      <div class="result-header">
        <div class="status-badge" :class="getBadgeClass">
          <i
            :class="
              scanResult.isCompleted ? 'el-icon-success' : 'el-icon-warning'
            "
            class="status-icon"
          ></i>
          <span>{{
            scanResult.status === "COMPLETED"
              ? $t("scanBarCodeCheck.results.flowCompleted")
              : $t("scanBarCodeCheck.results.flowIncomplete")
          }}</span>
        </div>
      </div>
      ...
      <el-card class="result-card" shadow="hover">
        <div class="info-section">
          <el-descriptions
            :title="$t('scanBarCodeCheck.results.barcodeInfo')"
            :column="1"
            border
          >
            <el-descriptions-item
              :label="$t('scanBarCodeCheck.results.barcode')"
            >
              {{ scanResult.barcode }}
            </el-descriptions-item>
            <el-descriptions-item
              :label="$t('scanBarCodeCheck.results.materialCode')"
            >
              {{ scanResult.materialCode }}
            </el-descriptions-item>
            <el-descriptions-item
              :label="$t('scanBarCodeCheck.results.materialName')"
            >
              {{ scanResult.materialName }}
            </el-descriptions-item>
            <el-descriptions-item
              :label="$t('scanBarCodeCheck.results.status')"
            >
              <el-tag
                :type="
              ...
              >
                {{
                  scanResult.status === "COMPLETED"
                    ? $t("scanBarCodeCheck.results.completed")
                    : $t("scanBarCodeCheck.results.incomplete")
                }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item
              :label="$t('scanBarCodeCheck.results.progress')"
            >
              <el-progress
                :percentage="scanResult.progress"
                :status="scanResult.status === 'COMPLETED' ? 'success' : ''"
              ></el-progress>
            </el-descriptions-item>
          </el-descriptions>
        </div>
        ...
        <div class="node-statistics">
          <div class="statistic-item">
            <div class="statistic-title">
              {{ $t("scanBarCodeCheck.statistics.totalNodes") }}
            </div>
            <div class="statistic-value">{{ scanResult.totalNodes }}</div>
          </div>
          <div class="statistic-item">
            <div class="statistic-title">
              {{ $t("scanBarCodeCheck.statistics.completedNodes") }}
            </div>
            <div class="statistic-value success">
              {{ scanResult.completedNodes }}
            </div>
          </div>
          <div class="statistic-item">
            <div class="statistic-title">
              {{ $t("scanBarCodeCheck.statistics.pendingNodes") }}
            </div>
            <div
              class="statistic-value"
              :class="{ warning: scanResult.pendingNodes > 0 }"
            >
        ...
        <div
          v-if="
            scanResult.pendingNodes > 0 &&
            scanResult.pendingNodesList.length > 0
          "
          class="pending-nodes-section"
        >
          <h3>{{ $t("scanBarCodeCheck.statistics.pendingNodesList") }}</h3>
          <el-table
            :data="scanResult.pendingNodesList"
            style="width: 100%"
            size="medium"
            :row-key="getRowKey"
          >
            <el-table-column
              prop="nodeName"
              :label="$t('scanBarCodeCheck.statistics.nodeName')"
            ></el-table-column>
            <el-table-column
              prop="nodeType"
              :label="$t('scanBarCodeCheck.statistics.nodeType')"
            >
...
export default {
  ...
  methods: {
    ...
    getStatusText(status) {
      ...
      if (!this.scanResult) return "";
      return this.scanResult.isCompleted
        ? this.$t("scanBarCodeCheck.results.flowCompleted")
        : this.$t("scanBarCodeCheck.results.flowIncomplete");
    },
    ...
    async checkBarcode() {
      if (!this.barcode) {
        this.$message.warning(
          this.$t("scanBarCodeCheck.messages.pleaseInputBarcode")
        );
        return;
      }
      ...
      try {
        let cleanValue = this.barcode;
        if (cleanValue.includes("DCZZ-")) {
          // 先检查是否为自制条码
        ...
        const response = await checkBarcodeCompletion(cleanValue);
        ...
        if (response.success && response.code === 200) {
          ...
          if (this.scanResult.isCompleted) {
            this.$message.success(
              this.$t("scanBarCodeCheck.messages.checkComplete")
            );
            this.showPopup = true;
            this.popupType = "ok";
            playAudio("lcywc");
          ...
          this.errorMessage =
            response.message ||
            this.$t("scanBarCodeCheck.messages.queryFailed");
